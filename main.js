// Create AquaFlow Professional Loader
const aquaLoader = document.createElement('div');
aquaLoader.id = 'aqua-loader';
aquaLoader.innerHTML = `
    <div class="loader-content">
        <i class="fas fa-faucet faucet-icon"></i>
        <div class="water-drip"></div>
        <div class="ripple-puddle"></div>
        <div class="loading-text">
            <span>L</span><span>O</span><span>A</span><span>D</span><span>I</span><span>N</span><span>G</span><span>...</span>
        </div>
    </div>
`;
document.body.prepend(aquaLoader);
    ...
    navLinks.forEach(link => {
        ...
        if (linkPath === currentPath) {
            ...
            if (link.classList.contains('dropdown-item')) {
                ...
                if (parent) {
                    parent.querySelector('.navbar-link').classList.add('active');
                }
            }
        }
    });
...
window.addEventListener('load', function () {
    const loader = document.getElementById('aqua-loader');
    if (loader) {
        // Shorter load time for professional feel
        setTimeout(() => {
            loader.classList.add('hidden');
            // We no longer remove the loader so we can reuse it for page transitions
        }, 800);
    }
...
function initPageTransitions() {
    ...
    document.addEventListener('click', function (e) {
        ...
        // Check if clicked element is a link and has an href
        if (link && link.href) {
            const targetUrl = new URL(link.href);
            const currentUrl = new URL(window.location.href);

            // Filter out external links, hash links, new tab links, and same page links
            if (
                targetUrl.origin === currentUrl.origin && // Internal link
                targetUrl.pathname !== currentUrl.pathname && // Different page
                !link.hash && // Not just a hash link (optional, depends on if you want transition for hash jumps)
...
function initThemeToggle() {
    ...
    // Setup click handlers for all theme toggles
...
window.addEventListener('load', function () {
    ...
    // Zerotrillion Effects: Apply to all pages EXCEPT Home 1 (which has class 'home-standard')
    const isHome1 = document.body.classList.contains('home-standard');
    ...
    if (!isHome1) {
        ...
        potentialCards.forEach(el => {
            ...
            el.style.animation = 'none'; // Clear animations
        });

        initTextScrambleAnimations();
        initImagePixelReveal();
        initCardPixelReveal();
    } else {
        // Home 1: Water Fill Effect
        /* Remove delayed timeout if not needed, or keep for consistency */
        setTimeout(() => {
            initWaterFillAnimations();
...
function initPageLoadAnimations() {
    ...
    cards.forEach((card, index) => {
        // Skip for Home 1 (water fill) AND others (pixel reveal)
        // Only apply default fade-in if we explicitly want it, or maybe just remove this entire block
        // to let specific revealers handle everything. 
        // For now, let's just RETURN for everyone so specific logic takes over.
        return;
...
function initHeroParticles() {
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        const duration = Math.random() * 15 + 10; // 10s to 25s float duration
...
document.addEventListener('DOMContentLoaded', () => {
    // Disabled: HeroAnimation creates colorful animated lines
    // new HeroAnimation();
});
...
function initServiceAreaCards() {
    ...
    areaCards.forEach(card => {
        ...
        card.addEventListener('click', function () {
            ...
            alert(`AquaFlow Tech provides comprehensive plumbing services in ${area}. Click OK to schedule a visit or call (555) 123-4567 for immediate assistance!`);
        });
    });
}
...
function initInteractiveMap() {
    ...
    if (officeMarker) {
        ...
        officeMarker.addEventListener('click', function (e) {
            e.stopPropagation();
            alert('AquaFlow Tech Office\n123 Main Street\nYour City, ST 12345\n\nClick OK to get directions!');
        });

        officeMarker.addEventListener('mouseenter', function (e) {
            showTooltip(e, 'Click for directions to AquaFlow Tech Office');
...
function initTextScrambleAnimations() {
    const observerOptions = {
        threshold: 0.5,
        rootMargin: '0px 0px -20px 0px'
    };

    const targetElements = document.querySelectorAll('.hero-title, .section-title, .card-title, .stats-number, .btn, h1, h2, h3, h4');
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (!el.classList.contains('scramble-done')) {
                    const scrambler = new TextScramble(el);
                    scrambler.setText(el.dataset.originalText);
    ...
    const revealObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                wrapper.classList.add('pixel-revealed');
                revealObserver.unobserve(wrapper);
            }
        });
    }, observerOptions);

    // Target specific images that should have this effect
    const targetImages = document.querySelectorAll('.card img, .hero-image-wrapper img, .home2-hero-image img, .feature-card img');

    targetImages.forEach(img => {
        // Skip if already wrapped
        if (img.parentElement.classList.contains('pixel-image-wrapper')) return;

        // Create Wrapper
        const wrapper = document.createElement('div');
        wrapper.className = 'pixel-image-wrapper';

        // Copy critical styles to ensure layout doesn't break
        wrapper.style.width = img.style.width || '100%';
        wrapper.style.height = img.style.height || 'auto';
        // If image has border radius, apply to wrapper
        ...
        // Create 100 blocks (10x10)
        const mask = document.createElement('div');
        mask.className = 'pixel-mask-grid';
        ...
        for (let i = 0; i < 100; i++) {
            ...
            // Random Delay
            // Mix of row delay (sweep down) + random noise
            const row = Math.floor(i / 10);
            const col = i % 10;
            const delay = (row * 0.05) + (col * 0.02) + (Math.random() * 0.3);
...
// Pixel alignment animation logic
function initPixelAlignAnimations() {
    const observerOptions = {
        threshold: 0.15,
        rootMargin: '0px 0px -50px 0px'
    };
    ...
    const pixelObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                entry.target.classList.add('pixel-visible');
                pixelObserver.unobserve(entry.target);
            }
        });
    }, observerOptions);

    const targetCards = document.querySelectorAll('.card, .stats-card, .service-card, .feature-card, .pricing-card, .testimonial-card, .process-step, .home2-feature-card, .service-area-card');

    // Group by parent for staggering
    ...
    containers.forEach((cards) => {
        cards.forEach((card, index) => {
            // Clean legacy animations
            card.classList.remove('fade-in-up', 'animate-on-scroll', 'animated');
            card.style.opacity = '';
            card.style.transform = '';
            card.style.animation = 'none';

            card.classList.add('pixel-align-card');

            // Row-based stagger (assuming ~3 cards per row)
            const staggerIndex = (index % 3) + 1;
            card.classList.add(`pixel-delay-${staggerIndex}`);

            pixelObserver.observe(card);
        });
    });
}

// Card Pixel Reveal Animation (Full Card Overlay)
function initCardPixelReveal() {
    const observerOptions = {
        threshold: 0.15,
        rootMargin: '0px 0px -50px 0px'
    };
    ...
    const cardObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                entry.target.classList.add('pixel-revealed');

                // Allow CSS hover effects to work again after animation
                // Max delay is approx 0.8s (row*0.1 + col*0.05 + random), plus 0.4s duration = ~1.2s
                setTimeout(() => {
                    entry.target.style.transform = '';
                    entry.target.style.transition = '';
                    entry.target.style.animation = '';
                    entry.target.style.opacity = ''; // Let CSS control opacity
                }, 1200);
    ...
    const targetCards = document.querySelectorAll('.card, .stats-card, .service-card, .feature-card, .pricing-card, .testimonial-card, .process-step, .home2-feature-card, .service-area-card, .dashboard-card');
    ...
    targetCards.forEach(card => {
        ...
        card.style.transition = 'none'; // Temporarily disable transition to prevent ghosting
        card.classList.remove('fade-in-up', 'fade-in', 'animate-on-scroll', 'animated');
        ...
        for (let i = 0; i < 25; i++) {
            ...
            // Random + Directional Delay
            const row = Math.floor(i / 5);
            const col = i % 5;
            const delay = (row * 0.1) + (col * 0.05) + (Math.random() * 0.2);
...
// Water Fill Animation (Home 1)
function initWaterFillAnimations() {
    const observerOptions = {
        threshold: 0.15,
        rootMargin: '0px 0px -50px 0px'
    };
    ...
    const waterObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                setTimeout(() => {
                    entry.target.classList.add('water-filled');
                    entry.target.classList.remove('waiting-reveal');
                }, delay);
    ...
    const targetCards = document.querySelectorAll('.card, .stats-card, .service-card, .feature-card, .testimonial-card, .process-step, .operations-card, .metric-card');

    targetCards.forEach(card => {
        // Remove generic scroll animations to avoid conflict
        card.classList.remove('pixel-revealed', 'fade-in-up', 'animate-on-scroll', 'animated');

        // Clear conflicting styles
        card.style.opacity = '';
        card.style.animation = 'none';

        // Capture the original border radius for manufacture in animation
        const style = getComputedStyle(card);
        const borderRadius = style.borderRadius;
        card.style.setProperty('--target-radius', borderRadius);

        card.classList.add('water-fill-card');
        card.classList.add('waiting-reveal'); // Use safe class
        waterObserver.observe(card);
    });

    // SAFETY FALLBACK: Ensure content is never permanently hidden
    setTimeout(() => {
        document.querySelectorAll('.waiting-reveal').forEach(el => {
            // Force reveal if it's been waiting too long (3s)
            el.classList.remove('waiting-reveal');
            el.classList.add('water-filled');
        });
    }, 3000);
}

// Water Droplet Trail Effect for Cards
function initWaterDropletTrail() {
    const cards = document.querySelectorAll('.card, .stats-card, .operations-card, .metric-card, .testimonial-card, .feature-card, .service-card');
    ...
    cards.forEach(card => {
        ...
        card.addEventListener('mousemove', function (e) {
            const now = Date.now();
            if (now - lastDropletTime < 50) return; // Limit creation rate
            lastDropletTime = now;
            ...
            droplet.className = 'water-droplet-trail';
...
// Create splash particles when water reaches bottom
function createSplashParticles(card, container) {
    const cardRect = card.getBoundingClientRect();
    const containerRect = container.getBoundingClientRect();
    const splashY = containerRect.bottom - cardRect.top;

    // Create 8-12 splash particles shooting outward
    const particleCount = Math.random() * 4 + 8;
    for (let i = 0; i < particleCount; i++) {
        const particle = document.createElement('div');
        particle.className = 'water-splash-particle';
        ...
        // Random angle for splash direction (outward and upward)
        const angle = (Math.PI * 2 * i) / particleCount + (Math.random() - 0.5) * 0.5;
        const distance = Math.random() * 80 + 40; // 40-120px distance
        const tx = Math.cos(angle) * distance;
        const ty = -Math.abs(Math.sin(angle) * distance); // Negative = upward

        particle.style.setProperty('--tx', tx + 'px');
        particle.style.setProperty('--ty', ty + 'px');

        // Splash animation
        particle.style.animation = `splashBurst 0.6s ease-out forwards`;
...
// Create bubble particles rising from water
function createBubbleParticles(card, container) {
    const bubbleCount = Math.random() * 6 + 4; // 4-10 bubbles
    ...
    for (let i = 0; i < bubbleCount; i++) {
        ...
        const randomX = Math.random() * (container.offsetWidth - size);
        bubble.style.left = randomX + 'px';
        bubble.style.bottom = '5px';

        // Random horizontal drift
        const bx = (Math.random() - 0.5) * 40; // -20 to +20px drift
        bubble.style.setProperty('--bx', bx + 'px');

        // Staggered animation
        const duration = Math.random() * 1 + 1.5; // 1.5-2.5s
        const delay = i * 0.1; // Stagger bubbles
        bubble.style.animation = `bubbleRise ${duration}s ease-out ${delay}s forwards`;

        container.appendChild(bubble);

        // Cleanup
        setTimeout(() => bubble.remove(), (duration + delay) * 1000 + 500);
    }
}

// Initialize water fill cards with splash effects
function initWaterFillCards() {
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting && !entry.target.classList.contains('water-animated')) {
                const card = entry.target;
                const container = card.querySelector('.water-fill-container');

                if (container) {
                    card.classList.add('water-animated');

                    // Start water droplet animation
                    let dropletCount = 0;
                    const maxDroplets = 12;
                    const dropletInterval = setInterval(() => {
                        if (dropletCount >= maxDroplets) {
                        ...
                        const randomX = Math.random() * (card.offsetWidth - 10) + 5;
                    ...
                    // Animate water fill after droplets start
                    setTimeout(() => {
                        let fillHeight = 0;
                        const fillInterval = setInterval(() => {
                            if (fillHeight >= 100) {
                                ...
                                // Trigger splash effect when water is full
                                createSplashParticles(card, container);

                                // Create rising bubbles shortly after splash
                                setTimeout(() => {
                                    createBubbleParticles(card, container);
                                }, 150);

                                return;
                            }
                            fillHeight += 2;
                            container.style.height = fillHeight + '%';

                            // Trigger splash at ~90% fill (preview splash)
                            if (fillHeight === 90) {
                                // Small pre-splash effect
                                const smallBubbles = Math.random() * 2 + 1;
                                for (let i = 0; i < smallBubbles; i++) {
                                    ...
                                    bubble.style.bottom = '5px';
                                    bubble.style.setProperty('--bx', (Math.random() - 0.5) * 20 + 'px');
                                    bubble.style.animation = `bubbleRise 1s ease-out forwards`;
                                    container.appendChild(bubble);
    ...
    waterFillCards.forEach(card => observer.observe(card));
}
